...
def test_user_starts_with_zero_credits():
    store = TradeCreditStore(db_path="/tmp/test_trade_credits_1.sqlite3")
    assert store.get_credit_balance("user-123") == 0.0


def test_credits_earned_from_trade_value():
    store = TradeCreditStore(db_path="/tmp/test_trade_credits_2.sqlite3")
...
def test_credits_spent_on_trade():
    store = TradeCreditStore(db_path="/tmp/test_trade_credits_3.sqlite3")
...
def test_insufficient_credits_raises():
    store = TradeCreditStore(db_path="/tmp/test_trade_credits_4.sqlite3")
...
def test_transaction_history():
    store = TradeCreditStore(db_path="/tmp/test_trade_credits_5.sqlite3")
